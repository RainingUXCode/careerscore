...
export const validationService = {
  ...
  validarCurriculo(arquivo?: File): string | null {
    if (!arquivo) return 'Anexe seu currículo em PDF.'
    const nome = arquivo.name.toLowerCase()
    const extensaoValida = nome.endsWith('.pdf')
    if (!extensaoValida) return 'Formato inválido. Envie um arquivo PDF.'
    if (arquivo.type && arquivo.type !== 'application/pdf') return 'Formato inválido. Envie um arquivo PDF.'
    if (arquivo.size === 0) return 'O arquivo PDF está vazio.'
