import type { JobProvider, FiltroBuscaVagas } from '../types/jobProvider'
...
export interface OpcoesBuscaVagas {
  /** Ignora o cache e força uma nova consulta às fontes — usado pelo botão "Atualizar vagas". */
  ...
  async buscar(filtros: FiltroBuscaVagas, opcoes: OpcoesBuscaVagas = {}): Promise<ResultadoAgregacaoVagas> {
    ...
    const execucoes = await Promise.all(
      this.providers.map(async (provider) => {
        try {
          const resultado = await provider.buscar(filtros)
    ...
    // falha temporária (cota, timeout) não deve "travar" o app em modo
    // demonstração por 24h; na próxima busca ele tenta a fonte real de novo.
    if (algumProviderRealTeveSucesso) {
      vagasCacheService.salvar(filtros, vagas)
    }

    return { vagas, fontesComFalha, codigosErro: Array.from(codigosErro), usouFallback, deCache: false, consultadoEm }
  }
}
